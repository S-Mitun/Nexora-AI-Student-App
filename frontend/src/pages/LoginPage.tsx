...
import { Layers, Mail, Lock, AlertCircle, ArrowRight, ShieldCheck, Eye, EyeOff, CheckCircle2 } from 'lucide-react';
...
export const LoginPage: React.FC = () => {
  const { signIn, signInWithGoogle, resendVerificationEmail, isAuthenticated, isSupabaseConnected } = useAuth();
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      if (error.message.includes('Invalid login credentials')) {
        ...
        setErrorMsg('Email verification required. Please confirm your email address before signing in.');
      } else {
  ...
  const handleResendVerification = async () => {
    ...
    if (error) {
      ...
      setResendSuccess(`Confirmation email resent to ${email.trim()}. Check your inbox.`);
    }
  };
      ...
      <Card className="w-full max-w-md border-nexora-border/80 shadow-glow-sm z-10">
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="success" dot>Student Portal</Badge>
          </div>
          <CardDescription>
            Access your personalized learning environment, interactive simulations, and notes.
          </CardDescription>
        </CardHeader>
                ...
                <div className="flex-1">
                  ...
                  {isUnconfirmed && (
                    <button
                      ...
                      className="block mt-2 font-semibold text-nexora-accent underline hover:text-white transition-colors"
                    >
        ...
        <CardFooter className="flex flex-col gap-3 pt-3 border-t border-nexora-border/40 text-center">
          <p className="text-xs text-nexora-text-muted">
            New to NEXORA?{' '}
            <Link to="/signup" className="text-nexora-accent hover:underline font-semibold">
          ...
          <div className="flex items-center justify-center gap-1.5 text-[11px] text-nexora-text-subtle">
            ...
            <span>Encrypted &amp; Secure Student Access</span>
          </div>
        </CardFooter>
      </Card>
    </div>
